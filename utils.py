...
def valid_keypoints_plot(image, outputs, orig_keypoints, epoch):
    ...
    random_index = random.randrange(0,len(image))
    ...
    for pred, target in zip(outputs_tensor, keypoints_tensor):
        ...
        for points in target.reshape(-1, 2):
            ...
            for corners in CORNERS:
                euclid_distance = torch.sqrt(
                    torch.pow(corners[0] - points[0], 2) + torch.pow(corners[1] - points[1], 2))
                if euclid_distance > max_dist:
                    max_dist = euclid_distance

            dist_pred_target = torch.sqrt(torch.pow(pred.reshape(-1, 2)[num_point][0] - points[0], 2) + torch.pow(
                pred.reshape(-1, 2)[num_point][1] - points[1], 2))
        ...
        if data_points is not None:
            if len(data_points) == 3:
                ...
                for batchSize, Color, fps_points in zip(config.INFER_BATCH_SIZES, colors, data_points[0]):
                    ax.plot(batchSize, fps_points, marker='o', markerfacecolor=Color, markeredgecolor='black',
                            markersize=15, label=f'BATCH SIZE: {batchSize}')
