...
class KeypointEfficientNet():
    def __init__(self, pretrained, requires_grad, model_name='efficientnet-b0'):
        if model_name not in ['efficientnet-b0', 'efficientnet-b1', 'efficientnet-b2', 'efficientnet-b3',
...
class KeypointCustom():
    def __init__(self, isPretrained, requires_grad, fineTuning=False, model_name='tf_efficientnet_lite0'):
            ...
            if fineTuning:
                for name, param in self.model.named_parameters():
                    if 'classifier' not in name:
                        param.requires_grad = False
                print("Fine tuning....just training the last layer")
            else:
